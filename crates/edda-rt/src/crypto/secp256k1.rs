//! ECDSA over secp256k1 (`std.crypto.sig.secp256k1`, SHA-256, RFC 6979),
//! backed by `k256`. Shares the 2-byte `Result<(),SigError>` / `Result<bool,_>`
//! wire shapes with the NIST ECDSA module (`Secp256k1SigError` is variant-
//! identical to `EcdsaSigError`).
...
/// secp256k1 ECDSA sign, DER output.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_sign_der(
...
/// secp256k1 ECDSA sign, raw r||s output.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_sign_raw(
    private_scalar: EdSlice,
    message: EdSlice,
...
/// secp256k1 ECDSA verify, DER signature.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_verify_der(
    public_key: EdSlice,
    message: EdSlice,
    signature: EdSlice,
) -> ResultBool {
...
/// secp256k1 ECDSA verify, raw r||s signature.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_verify_raw(
    public_key: EdSlice,
    message: EdSlice,
    signature: EdSlice,
) -> ResultBool {
...
/// secp256k1 point compression (65 → 33 bytes).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_point_compress(
...
/// secp256k1 point decompression (33 → 65 bytes).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_point_decompress(
    compressed: EdSlice,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __edda_secp256k1_public_from_private(
    private_scalar: EdSlice,
    public_uncompressed_out: *mut EdSlice,
) -> ResultUnit {
    // SAFETY: slices satisfy the crypto-slice-borrow contracts.
    let (scalar, out) = unsafe { (in_bytes(private_scalar), out_bytes(public_uncompressed_out)) };
    let Ok(sk) = SigningKey::from_slice(scalar) else {
        return err_unit(ERR_PRIV);
    };
